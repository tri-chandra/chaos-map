<template>
  <g>
      <path :d="pathString" @click="$emit('click', node)"
        ...
        stroke-width="1.5px"
      />
      <text :x="x+margin" :y="y+length+margin" color="#000000" font-size="xx-small">[{{node.text}}]</text>
    </g>
</template>

<script>
export default {
  name: 'hexagon-monument',
  props: {
  ...
  computed: {
      pathString() {
        ...
        const path = `M${this.x} ${this.y} h${this.length}` +
            ...
            `l${-this.length / 2} ${Math.round(this.length * Math.cos(A))}` +
            `l${this.length / 2} ${Math.round(this.length * Math.cos(A))}` +
            `v${-this.length * 2 * Math.cos(A)}` +
            `m${this.length} 0v${this.length * 2 * Math.cos(A)}`;
